...
4. **采集数量控制与安全选择题交互设计**
   - **默认/推荐单次采集 5 条**（`--limit=5`）。小红书的风控机制极其严厉，如果频繁、批量采集极易导致账号被限制或封禁。**特别是当用户使用个人主力大号登录时，5 条是保障账号绝对安全的红线！**
   - **选择题交互设计：** 在向用户确认开始采集前，AI **必须**向用户呈现一个清晰的选择题式交互（Selector），明确说明安全级别与限制原因，让用户主动选择或确认风险：

     > 💡 **【账号安全与数据量选项】**
     > 为了保障您的小红书账号安全、彻底规避封号风险，请问您当前在 Chrome 浏览器中登录的是什么类型的账号？
     > 
     > - 1️⃣ **【安全推荐】主力大号 / 个人日常号** (推荐 ⭐⭐⭐⭐⭐)
     >   * **采集数量：** 5 条 (`--limit=5`)
     >   * **安全等级：** 极高（完全模拟真人阅读速度，对您的主力大号绝对安全）
     >   * **适用场景：** 快速连通测试、小规模体验痛点与分析效果
     > 
     > - 2️⃣ **【深度分析】测试小号 / 专门采集废号** (推荐指数 ⭐⭐⭐)
     >   * **采集数量：** 20 条 (`--limit=20`)
     >   * **安全等级：** 中等（由于数量较多，存在一定的限流或风控风险，建议仅在小号上使用）
     >   * **适用场景：** 需要充足的样本量，供「好痛 Howtone」进行深度的商机提炼与生成完整报告
     > 
     > 👉 **请直接回复 [1] 或 [2] 告诉您的选择，我将为您配置对应的安全参数启动采集！**

   - **执行逻辑：**
     * 用户回复 `1`：以 `--limit=5` 执行采集脚本。
     * 用户回复 `2`：以 `--limit=20` 执行采集脚本。
...
# 基础用法 (极度推荐：默认安全采集 5 条，保护账号防封控)
npx ts-node webbridge-crawl.ts --keyword="关键词" --limit=5

# 高级用法 (仅在用户使用测试小号、且需要更丰富数据供「好痛」深度分析时使用)
npx ts-node webbridge-crawl.ts --keyword="关键词" --limit=20

# 不采评论
npx ts-node webbridge-crawl.ts --keyword="关键词" --limit=5 --no-comments

# 限制评论数
npx ts-node webbridge-crawl.ts --keyword="关键词" --limit=5 --max-comments=10
```
